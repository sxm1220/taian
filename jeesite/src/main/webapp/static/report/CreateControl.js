...
var gr_InstallPath = "/resources/uploadFile"; //实际项目中应该写从根目录寻址的目录，如gr_InstallPath="/myapp/report/grinstall"; 
...
//同步
function AjaxSyncLoadReportData(Report, DataUrl, RunFun,dataStr)
{
	...
	if(dataStr!=null){
		dataStr += "&"+Math.random();
	    xmlhttp.open("GET", DataUrl+dataStr, false);
	}else{
...
function AjaxReportRun(Report, DataUrl, RunFun,dataStr) 
{
    ...
    if(dataStr!=null){
    	dataStr += "&"+Math.random();
        xmlhttp.open("GET", DataUrl+dataStr, true);
    }else{
...
function Install_Detect()
{
    ...
    if (_ReportOK.Register == undefined) //if ((_ReportOK == null) || (_ReportOK.Register == undefined))
    {
        ...
        if( _gr_isIE )
            ...
            document.write('锐浪报表插件安装后，<a href="javascript:void(0)" onclick="demo()">点击此处</a> 重新加载此网站');
        document.write('</div>');
        return false;
    }
